...
describe('AdminDashboard', () => {
  ...
  it('renders dashboard donut analytics without the old standalone analytics card', async () => {
    ...
    expect(screen.getByText('Admin Dashboard').parentElement).toHaveClass('flex', 'flex-col', 'gap-6')
    ...
    expect(screen.getByRole('region', { name: /Current Round/i })).toHaveClass('overflow-hidden', 'rounded-2xl', 'border', 'border-l-4', 'bg-white', 'shadow-sm')
    expect(screen.getByRole('region', { name: /Current Round/i })).toHaveStyle({ borderLeftColor: '#16a34a' })
    expect(screen.getByText('Period')).toBeInTheDocument()
    expect(screen.getByText('Completion Rate').closest('.grid')).toHaveClass('grid-cols-2', 'gap-4')
    ...
    expect(screen.getByRole('link', { name: /Users/i }).parentElement).toHaveClass('grid-cols-2', 'md:grid-cols-3')
